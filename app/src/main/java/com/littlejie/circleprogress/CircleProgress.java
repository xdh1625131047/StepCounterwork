...
import com.tony.stepcounter.BuildConfig;
...
public class CircleProgress extends View {
    ...
    private void initPaint() {
        ...
        mValuePaint.setShadowLayer(3, 5, 5, mContext.getResources().getColor(R.color.black));
        ...
        mBgArcPaint.setShadowLayer(3, 5, 5, mContext.getResources().getColor(R.color.black));
    }
...
//    	spannableString.setSpan(span, 2, 4, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
//
//    	SpannableStringBuilder stringBuilder = new SpannableStringBuilder();
//    	String stringToAppend = "hello world";
//    	stringBuilder.append(stringToAppend);
//    	stringBuilder.setSpan(span, 2, 4, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);

        Resources res = mContext.getResources();
        canvas.drawText(String.format(mPrecisionFormat + res.getString(R.string.str_step), mValue), mCenterPoint.x, mValueOffset, mValuePaint);
//        canvas.drawText("100000steps", mCenterPoint.x, mValueOffset, mValuePaint);

        if (mHint != null) {
            canvas.drawText(mHint, mCenterPoint.x, mHintOffset, mHintPaint);
        }

        if (mUnit != null) {
            canvas.drawText(String.format(res.getString(R.string.str_circle_progress_bar_unit_display), mUnit), mCenterPoint.x, mUnitOffset, mUnitPaint);
...
//        }
        ...
        if (0 < mMaxValue) {
            mUnit = String.format("%.1f%%", value / mMaxValue*100);
...
//                mValue = mPercent * mMaxValue;
    ...
    public void setMaxValue(float maxValue) {
        ...
        if (0 < mMaxValue) {
            mUnit = String.format("%.1f%%", mValue / mMaxValue*100);
            float start = mPercent;
            float end = mValue / mMaxValue;
            startAnimator(start, end, mAnimTime);
        }
    }
